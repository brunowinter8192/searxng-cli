...
Bead: `searxng-f3i` | Predecessor investigation: `scholar_concurrent_block_20260508.md`

---

## Status Quo (IST — before this change)
...
- Production result: 100% EMPTY_BLOCK in every default or pdf-mode query (triggers a+b both active — see `scholar_concurrent_block_20260508.md` Evidence Table)
...
- Scholar dormant in all default production paths until Pooling-Rework (bead `searxng-g82`)
...
- If 1.5s insufficient, smoke fails → tune up toward slowest-engine ceiling (6s open_library). At N≥3s the delay exceeds Scholar HTTP search time (~700ms median) and becomes visible in wallclock — degrades from Rang 1 (absorbed) to Rang 2 (visible) per `fast-by-default.md` Mitigation-Choice Discipline. Rang 2 requires explicit user approval.
...
- (c) is deterministic: `_DEFAULT_ENGINES` is a declarative constant, no runtime conditional, no timing hypothesis, no tuning budget. Pooling-Rework (bead g82) gets a clean slate — add a new constant, point Scholar at it.

## Why Not Option (a) or (b)

- **(a) Sequential Scholar after gather** — +700ms unconditional on every academic-with-google query. `total_ms = max(other_engines) + scholar_http` instead of `max(other_engines)`. `fast-by-default.md` Rang 2: requires explicit user acceptance of wallclock cost. Not justified when Rang 3 (Scholar dormant, zero coverage delta from current 100%-blocked state) is available.
...
## Pooling-Übergabe to bead `searxng-g82`
...
1. Pool definition for g82 must place Scholar in a **Google-free pool** — a named engine set constant that excludes `"google"`.
...
3. The `_DEFAULT_ENGINES` constant in `filter_modes.py` is the correct extension point — g82 adds new constants alongside it, does not touch `_DEFAULT_ENGINES`.
4. This file is the IST reference for g82 design notes when that work starts.
...
| Source | Relevance |
|---|---|
| `decisions/OldThemes/scholar_concurrent_block_20260508.md` | Root-cause investigation, 2×2 architecture × Google matrix, Evidence Table |
...
| `~/.claude/shared-rules/proj_searxng/fast-by-default.md` | Mitigation-Choice Discipline — Rang 1/2/3 hierarchy, rationale for choosing (c) over (d)/(a) |
| Bead `searxng-ciw` | HTTP migration work (Step 1) |
| Bead `searxng-f3i` | Google decoupling work (Step 2, this change) |
| Bead `searxng-g82` | Pooling-Rework — Scholar's future Google-free pool home |
