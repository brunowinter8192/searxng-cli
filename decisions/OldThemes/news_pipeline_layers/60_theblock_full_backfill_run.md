...
The 22,995 pre-OT52 raw HTML files lived in `data/news/theblock/scrape/` (the old pre-decoupling location, OT51). The current pipe dedups against `data/news/theblock/raw/` — `filter_new_entries(..., mode="raw")` checks `raw/{hash}.md` existence. `raw/` did not exist → a `--timeframe full` run would have found raw/ empty and re-scraped all ~26,955 discovered URLs from scratch.
...
Then issue #11 can close. The failures-exclusion (OT59) is a no-op for this first run — no dead/failed lists existed at its dedup time, they are created during it — and takes effect from the second run.

## Relationship to prior OTs
- OT52 — raw-only decoupling (cleanup removed from pipe). Superseded for cleanup by OT57 (clean-pass re-wired into the proxy_pool path; only indexing stays decoupled).
- OT55 — single master list. OT58 relocated it into `discover/`.
- OT56 — proxy_pool stall termination → source of `failed_urls.txt`.
- OT57 — in-pipe clean-pass (body-less log → `clean/bodyless_urls.txt`).
- OT59 — failure-URL diff exclusion.
