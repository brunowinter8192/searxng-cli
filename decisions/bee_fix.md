# bee_fix — RATE_WAIT_TIMEOUT + Bucket-Uniformity + Scholar Removal

## Status Quo (IST)

Three production changes applied 2026-05-21 (commit on branch `engine-uniformity`):
...
Three probe phases in `decisions/OldThemes/bee_cdp_starvation/`:
...
Keep (no change needed). Fix verified by smoke. The three changes together eliminate the cascade.
