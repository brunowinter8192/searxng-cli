...
### search_web.py (350 LOC)

**Purpose:** Search orchestrator. Fans out across the 13 active engines via `asyncio.gather`, then filters → builds pools → caps → formats → caches. Post-dedup pool cap: K = `len(pools['google'])` if >0 else 10, each pool trimmed to `pool[:K]` (prevents CrossRef/OpenAlex/StackExchange/OpenLibrary drilldown floods). Three-tier timeout: `ENGINE_WATCHDOG_TIMEOUT=3.6s` default, `ENGINE_WATCHDOG_OVERRIDE` per-engine (open_library 6.0, semantic_scholar 5.0, crossref 6.0, startpage 6.0, brave 6.0) — `bing` and `yandex` deliberately have NO override entry: both probed under ~2.1s clean, and `yandex` additionally short-circuits its own blocked-query path (an upfront `showcaptcha`-URL check before the result-wait poll) so a block never approaches the 3.6s default either. `RATE_WAIT_TIMEOUT=60.0s` acquire cap → RATE_SKIP. `_engine_with_timing` returns a 5-tuple `(results, rate_wait_ms, search_ms, status, drop_reason)` with sub-classified TIMEOUT/ERROR statuses. Two-record logging: `engine_run` after fanout, `workflow_summary` after pool-build. `fetch_search_results` is a sync dev wrapper (raw list, no pools).
