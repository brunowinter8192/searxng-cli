...
- `python -m src.news --source coindesk --scrape-only --year YYYY [--from YYYY-MM-DD] [--to YYYY-MM-DD] [--limit N] [--browsers N] [--slots N] [--cooldown-policy fixed|exp]` — date-filtered backfill: discover → dedup(raw) → proxy_riding scrape → raw persist → job report; `--browsers`/`--slots` override `RidingScrapeConfig` defaults (4 browsers, 64 slots) per-run without code edits; `--cooldown-policy fixed` (default, 60-min flat) or `exp` (exponential backoff with jitter, base=300s, cap=3600s, reset-on-productive-ride) for A/B proxy-supply experiments
