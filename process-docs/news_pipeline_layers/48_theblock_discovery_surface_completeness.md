...
Open question raised after the CoinDesk timeline-API work (OT44/47). NOT started — parked follow-up.
...
sub-sitemaps → ~47k article URLs (OT29 dev-run; OT43 convention). The sitemap was adopted as THE
discovery surface from the first session (OT14) and never compared against any alternative internal
...
~1 year (OT44), while its internal cursor-paginated timeline API yields 61,628 URLs back to 2017 (OT47)
...
- **Internal API probe.** Does theblock.co's frontend call an internal "load more" / timeline-style API
  ...
  sitemap? Capture via browser network/HAR on the article feed, as done for CoinDesk (OT44).
