...
**Date:** 2026-06-13
**State:** Design LOCKED (chat synthesis). Build is a fresh dev/ pipe; the dev-run can start on the existing curated pool, the backfill pool is pending the OldThemes-27 survey. Forward path after jhao104 was dropped (OldThemes 27).
...
**KEY decision: NO separate proxy-check stage. Every proxy request is a PRODUCTIVE theblock fetch.** A separate check would waste 1 per-IP-budget unit; with B as low as 1-4 (measured 4-20, OldThemes 21) that is wasteful-to-catastrophic (B=1 → proxy dead before any useful fetch). A proxy earns use by successfully fetching real content.
...
- **60min cooldown = the CF-block duration AND the dedup/staleness state** — don't retry a dead/burned proxy for 60min (its CF block lasts ~that long; CF mitigation_timeout 600-3600s per OldThemes 23).
...
| Pool weighting | socks4-favoured (best CF cohort, 3.57% vs http 0.94%, OldThemes 27) |
| Timeout | 15s |
...
**NO per-repo runtime attribution** — overlapping lists make per-source credit meaningless (OldThemes 24 prior). The survey is a one-time selection snapshot only.
...
**Concurrency = 128 (decided, not 50).** Candidate search runs at concurrency 128 (CLI param). Reasoning: ~128 is the grounded router-clean-zone (OldThemes 20) AND the pool-sizing was computed at 128. The 50 used for the SURVEY/probe was justified ONLY there (single-shot rate measurement, no retry → a router-false-timeout would bias the rate down). In the PIPE the retry logic absorbs false-timeouts (a false-timed-out URL just retries with the next proxy → nothing lost), so conservatism is pointless. 128 vs the heavy-theblock-GET clean zone is unmeasured → the testlauf logs the timeout-fraction to confirm; if 128 saturates (effective yield drops, like 512 did on light checks) tune DOWN data-driven.
...
**Backfill candidate pool = top 13 survey repos (~22k unique)** (OldThemes 27). socks4-favoured. NOT only-socks4 (would lose the http stars themiralay/monosans); socks5 is the weakest if we ever trim. First run: blast all protocols, log per-protocol, refine later.

## Pending — next session
- **Stage 4 testlauf** at concurrency 128 on the COLD curated pool: the 64-sitemap dev-run — measure real wall-time + observed B + timeout-fraction (128 saturation check).
