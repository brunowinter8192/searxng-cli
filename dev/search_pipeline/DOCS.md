...
| `24_pydoll_teardown_verify.py` | Integration test for the kill_tab teardown fix (bead 7u5). Navigates one tab to `chrome://hang` (freezes renderer), wraps in `asyncio.wait_for(5s)`, verifies: wall time < 8s (not the old ~65s), tab gone from `_browser._tabs_opened`, renderer count stable. Second test: normal `about:blank` tab through `kill_tab`. Output `teardown_verify_<ts>.md` to `01_reports/`. Uses `importlib` to access production `src/search/browser.py` functions directly. |
