...
# Build fresh active buffer: up to max_size eligible proxies from pool_22k, socks4-first
def build_active_buffer(
    pool_22k: list[tuple[str, str]],
    cm: PersistentCooldownManager,
    max_size: int = BUFFER_SIZE,
) -> list[tuple[str, str]]:
    """Return up to max_size eligible proxies from pool_22k in socks4-first order.

    Eligibility is delegated entirely to cm.eligible_candidates() which applies
    the wall-clock UTC cooldown check and socks4→socks5→http ordering.
    """
...
def refill_buffer(
    ...
    New additions are appended in socks4-first eligible order.
    """
